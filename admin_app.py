...
# ----------------------------
st.markdown("---")
st.subheader("💸 Promo code analytics (filtered trips)")

if not df_trips_filtered.empty and "promo_code" in df_trips_filtered.columns:
    ...
    if not df_promo.empty:
        ...
        csv_promo = promo_group.to_csv(index=False).encode("utf-8")
        st.download_button(
            label="Download promo summary as CSV",
            data=csv_promo,
            file_name="promo_summary_mali_ride.csv",
            mime="text/csv"
        )

        st.markdown("#### Trips by promo code (count)")
        ...
        st.info("No promo codes used for current filters.")
else:
...
# ----------------------------
st.markdown("---")
st.subheader("🎁 Referral code analytics (filtered trips)")

if not df_trips_filtered.empty and "referral_code" in df_trips_filtered.columns:
    ...
    if not df_ref.empty:
        ...
        ref_group["avg_fare_xof"] = ref_group["avg_fare_xof"].round(2)

        st.dataframe(ref_group)

        csv_ref = ref_group.to_csv(index=False).encode("utf-8")
        st.download_button(
            label="Download referral summary as CSV",
            data=csv_ref,
            file_name="referral_summary_mali_ride.csv",
            mime="text/csv"
        )

        st.markdown("#### Trips by referral code (count)")
        st.bar_chart(ref_group.set_index("referral_code")["trips_count"])
    else:
        st.info("No referral codes used for current filters.")
else:
    st.info("Referral code information not available in trips.")


# ----------------------------
# DRIVER PAYOUT REPORT (filtered trips)
# ----------------------------
st.markdown("---")
st.subheader("💵 Driver payout report (filtered trips)")

if not df_trips_filtered.empty and "driver_username" in df_trips_filtered.columns:
    df_pay = df_trips_filtered.copy()
    # Ensure datetime
    if "created_at" in df_pay.columns:
        df_pay["created_at"] = pd.to_datetime(df_pay["created_at"], errors="coerce")
        df_pay["trip_date"] = df_pay["created_at"].dt.date
        # Week start (Monday) for grouping
        df_pay["week_start"] = df_pay["created_at"].dt.to_period("W").apply(lambda r: r.start_time.date())
    else:
        df_pay["trip_date"] = pd.NaT
        df_pay["week_start"] = pd.NaT

    # Aggregate by driver (for current filter range)
    group_cols = ["driver_username"]
    if "driver_name" in df_pay.columns:
        group_cols.append("driver_name")

    agg_driver = df_pay.groupby(group_cols).agg(
        trips_count=("price_xof", "count"),
        total_revenue_xof=("price_xof", "sum"),
        total_driver_earnings_xof=("driver_earnings_xof", "sum"),
        total_platform_commission_xof=("platform_commission_xof", "sum"),
    ).reset_index()

    agg_driver["avg_fare_xof"] = (agg_driver["total_revenue_xof"] / agg_driver["trips_count"]).round(2)
    agg_driver["effective_commission_pct"] = (
        100 * agg_driver["total_platform_commission_xof"] / agg_driver["total_revenue_xof"].replace(0, pd.NA)
    ).round(2)

    st.markdown("#### By driver (current filters)")
    st.dataframe(agg_driver)

    csv_driver = agg_driver.to_csv(index=False).encode("utf-8")
    st.download_button(
        label="Download driver payout summary as CSV",
        data=csv_driver,
        file_name="driver_payout_summary_mali_ride.csv",
        mime="text/csv"
    )

    # Weekly payout view
    if "week_start" in df_pay.columns and df_pay["week_start"].notna().any():
        group_cols_week = ["week_start", "driver_username"]
        if "driver_name" in df_pay.columns:
            group_cols_week.append("driver_name")

        agg_week = df_pay.groupby(group_cols_week).agg(
            trips_count=("price_xof", "count"),
            total_revenue_xof=("price_xof", "sum"),
            total_driver_earnings_xof=("driver_earnings_xof", "sum"),
            total_platform_commission_xof=("platform_commission_xof", "sum"),
        ...
        agg_week["avg_fare_xof"] = (agg_week["total_revenue_xof"] / agg_week["trips_count"]).round(2)
        agg_week["effective_commission_pct"] = (
            100 * agg_week["total_platform_commission_xof"] / agg_week["total_revenue_xof"].replace(0, pd.NA)
        ).round(2)

        st.markdown("#### Weekly payout (by driver)")
        st.dataframe(agg_week)

        csv_week = agg_week.to_csv(index=False).encode("utf-8")
        st.download_button(
            label="Download weekly payout summary as CSV",
            data=csv_week,
            file_name="weekly_driver_payout_summary_mali_ride.csv",
            mime="text/csv"
        )
    else:
        st.info("Week information not available for weekly grouping.")
else:
    st.info("No driver information available for payout report.")
